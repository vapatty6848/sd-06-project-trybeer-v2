...
const dropDataBase = () => 'DROP DATABASE IF EXISTS Trybeer;';

const createDataBase = () => 'CREATE DATABASE IF NOT EXISTS Trybeer;';

const createTableUsers = () => {
  const tableUsers = `CREATE TABLE Trybeer.users (
...
const createTableProducts = () => {
  const tableProducts = `CREATE TABLE Trybeer.products (
    id INT NOT NULL AUTO_INCREMENT,
    name VARCHAR(100) NOT NULL,
...
const createTableSales = () => {
  const tableSales = `CREATE TABLE Trybeer.sales (
    id INT NOT NULL AUTO_INCREMENT,
...
const createTableSalesProducts = () => {
  const tableSalesProducts = `CREATE TABLE Trybeer.sales_products (
...
const createAndInsertsDataBase = async () => {
  await connection.execute(dropDataBase());
